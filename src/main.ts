...
async function main() {
	...
	for (const link of links) {
		// console.log("link", await link.locator("time").innerText());
		await link.click();
	...
	console.log(JSON.stringify(movieSchedules, null, 2));
...
function mergeSchedule(schedules: Schedule[]): Schedule[] {
	// const a = Object.groupBy(schedules, (s) => {s.});
	return [];
}
